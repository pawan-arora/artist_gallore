...
/// [SearchFloatingDelegate.close] call. If the search page is closed with the system
/// back button, it returns null.
///
/// A given [SearchFloatingDelegate] can only be associated with one active [showFloatingSearch]
/// call. Call [SearchFloatingDelegate.close] before re-using the same delegate instance
/// for another [showFloatingSearch] call.
///
...
/// The animation provided by [SearchFloatingDelegate.transitionAnimation] can be used
...
/// See also:
///
///  * [SearchFloatingDelegate] to define the content of the search page.
Future<T?> showFloatingSearch<T>({
  required BuildContext context,
  required SearchFloatingDelegate<T> delegate,
  String? query = '',
...
/// Delegate for [showFloatingSearch] to define the content of the search page.
///
/// The search page always shows an [AppBar] at the top where users can
/// enter their search queries. The buttons shown before and after the search
/// query text field can be customized via [SearchFloatingDelegate.buildLeading]
/// and [SearchFloatingDelegate.buildActions]. Additionally, a widget can be placed
/// across the bottom of the [AppBar] via [SearchFloatingDelegate.buildBottom].
///
/// The body below the [AppBar] can either show suggested queries (returned by
/// [SearchFloatingDelegate.buildSuggestions]) or - once the user submits a search  - the
/// results of the search as returned by [SearchFloatingDelegate.buildResults].
///
/// [SearchFloatingDelegate.query] always contains the current query entered by the user
/// and should be used to build the suggestions and results.
///
/// The results can be brought on screen by calling [SearchFloatingDelegate.showResults]
/// and you can go back to showing the suggestions by calling
/// [SearchFloatingDelegate.showSuggestions].
///
/// Once the user has selected a search result, [SearchFloatingDelegate.close] should be
/// called to remove the search page from the top of the navigation stack and
/// to notify the caller of [showFloatingSearch] about the selected search result.
///
/// A given [SearchFloatingDelegate] can only be associated with one active [showFloatingSearch]
/// call. Call [SearchFloatingDelegate.close] before re-using the same delegate instance
/// for another [showFloatingSearch] call.
...
/// ## Handling emojis and other complex characters
/// {@macro flutter.widgets.EditableText.onChanged}
abstract class SearchFloatingDelegate<T> {
  ...
  /// communicate the result back to the initial caller of [showFloatingSearch].
  ...
  /// to [showFloatingSearch] that launched the search initially.
...
enum _SearchBody {
  ...
  ///
  /// The suggested queries are generated by [SearchFloatingDelegate.buildSuggestions].
  suggestions,

  /// Search results are currently shown in the body.
  ///
  /// The search results are generated by [SearchFloatingDelegate.buildResults].
  results,
}
...
class _SearchPageRoute<T> extends PageRoute<T> {
  ...
  final SearchFloatingDelegate<T> delegate;
        ...
        child: Scaffold(
          appBar: AppBar(
            ...
            bottom: PreferredSize(
              ...
              child: widget.delegate.buildBottom(context)!,
            ),
          ),
